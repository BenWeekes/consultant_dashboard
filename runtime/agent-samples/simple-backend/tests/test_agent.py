...
class TestCreateAgentPayload:
    ...
    def test_pipeline_custom_llm_uses_inbound_secret(self, test_constants):
        """Custom LLM pipeline payloads should use a dedicated inbound secret."""
        constants = test_constants.copy()
        constants.update({
            "PIPELINE_ID": "pipeline_123",
            "LLM_VENDOR": "custom",
            "LLM_URL": "https://mindfix.me/chat/completions",
            "LLM_API_KEY": "stale_key_should_not_be_forwarded",
            "CUSTOM_LLM_INBOUND_SECRET": "custom-llm-inbound-secret",
            "DEFAULT_PROMPT": "You are a helpful assistant",
            "DEFAULT_GREETING": "Hello",
            "DEFAULT_FAILURE_MESSAGE": "Error",
            "MAX_HISTORY": 10,
        ...
        assert payload["properties"]["llm"]["api_key"] == "custom-llm-inbound-secret"
        assert payload["properties"]["llm"]["api_key"] != constants["LLM_API_KEY"]

    def test_standard_custom_llm_uses_inbound_secret(self, test_constants):
        """Standard custom LLM payloads should authenticate without forwarding the provider key."""
        constants = test_constants.copy()
        constants.update({
            "LLM_URL": "https://mindfix.me/chat/completions",
            "LLM_VENDOR": "custom",
        ...
        assert payload["properties"]["llm"]["api_key"] == "custom-llm-inbound-secret"
        assert payload["properties"]["llm"]["api_key"] != constants["LLM_API_KEY"]
        assert payload["properties"]["llm"]["params"]["rtm_uid"] == build_custom_llm_rtm_uid("test_channel")

    def test_custom_llm_requires_inbound_secret(self, test_constants):
        constants = test_constants.copy()
        constants.update({
            "LLM_URL": "https://mindfix.me/chat/completions",
            "LLM_VENDOR": "custom",
        ...
        with pytest.raises(ValueError, match="CUSTOM_LLM_INBOUND_SECRET"):
            create_agent_payload(
                channel="test_channel",
                constants=constants,
                query_params={},
    ...
    def test_custom_llm_rtm_uid_is_channel_scoped_and_numeric(self):
        """Custom LLM RTM UIDs should be stable per channel and avoid global collisions."""
        uid_one = build_custom_llm_rtm_uid("channel_one")
        uid_two = build_custom_llm_rtm_uid("channel_two")

        assert uid_one.isdigit()
        assert uid_two.isdigit()
        assert uid_one != uid_two
        assert uid_one == build_custom_llm_rtm_uid("channel_one")
