...
class ConsultantDashboardTest(unittest.TestCase):
    ...
    def test_build_prompt_addition_includes_dashboard_context(self):
        prompt = build_prompt_addition({
            ...
            "ai_personal_summary": {
                "key_point_summary": {
                    "headline": "Sleep and wine habits",
                    "body": "The client is motivated to improve sleep quality and reduce evening habits that may worsen rest.",
                }
            },
            "latest_summary": {
        ...
        }, audio_biomarkers_enabled=True, video_biomarkers_enabled=False, phone_number="+14155550123")

        self.assertIn("Background notes", prompt)
        self.assertIn("Focus on routines.", prompt)
        self.assertIn("Sleep and wine habits", prompt)
        self.assertIn("improve sleep quality", prompt)
